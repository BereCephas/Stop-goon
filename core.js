...
function handleDeleteSite(siteId) {
    console.log('Demande de suppression du site avec ID:', siteId);

    chrome.tabs.query({ active: true, currentWindow: true }, function(tabs) {
        if (tabs.length > 0 && tabs[0].id) {
            // Send message to the background script, including the tab ID
            chrome.runtime.sendMessage({
                action: "showConfirmationPopup",
                siteIdToDelete: siteId,
                tabId: tabs[0].id // Pass the tab ID
            }).then(response => {
                // If background script sends a response (e.g., error during injection)
                if (response && !response.success) {
                    alert(response.error);
                }
            }).catch(error => {
                console.error("Error sending message from popup to background:", error);
                alert("Erreur interne lors de la préparation de la suppression.");
            });
        } else {
            alert("Impossible de trouver un onglet actif pour afficher la confirmation.");
        }
    });
}

// Listen for messages from the background script (for deletion results)
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse) {
    if (request.action === "deletionResult") {
        if (request.success) {
            alert(request.message);
            loadSites(); // Recharger la liste des sites dans le popup
            showSuccessMessage(); // If you have this function
        } else {
            alert('Erreur: ' + request.error);
        }
    }
    // No need to return true unless core.js also needs to send an async response
});
